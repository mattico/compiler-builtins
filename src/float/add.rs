use core::{f32, f64};
use core::num::{Wrapping, FpCategory, Float as float};
use float::Float;
            ...
            match (a.classify(), b.classify()) {
                ...
                (cat_a @ _, cat_b @ _) => {
                    if b.abs() > a.abs() {
                        ...
                        ::core::mem::swap(&mut cat_a, &mut cat_b);
                    }
                    let (a_exponent, a_significand) = match cat_a {
                        FpCategory::Subnormal => <$ty>::normalize(a.exponent()),
                        FpCategory::Normal => (a.exponent() as i32, a.significand()),
                    };
                    let (b_exponent, b_significand) = match cat_b {
                    ...
                    let result_sign = (a.sign() as <$ty as Float>::Int) << bits;
                    ...
                    if align != 0 {
                        if align < bits as i32 {
                            let sticky = (b_significand.wrapping_shl((bits as i32).wrapping_sub(align) as <$ty as Float>::Int) != 0) as <$ty as Float>::Int;
                            b_significand = b_significand.wrapping_shr(align as <$ty as Float>::Int) | sticky;
                        } else {
                    ...
                    if subtraction {
                        ...
                        if a_significand < implicit_bit << 3 {
                            ...
                            a_significand = a_significand.wrapping_shl(shift as <$ty as Float>::Int);
                    ...
                    if a_exponent <= 0 {
                        ...
                        let sticky = (a_significand.wrapping_shl(
                            bits.wrapping_sub(shift as <$ty as Float>::Int) as <$ty as Float>::Int) != 0) as <$ty as Float>::Int;
                        a_significand = a_significand.wrapping_shr(shift) | sticky;
                        a_exponent = 0;
                    }
                    ...
                    result |= a_exponent.wrapping_shl(significand_bits as <$ty as Float>::Int) as <$ty as Float>::Int;
                    result |= (result_sign as <$ty as Float>::Int) << bits;
...
mod tests {
    ...
    #[bench]
    fn bench_add_float_normal(b: &mut Bencher) {
        b.iter(|| {
            let n = test::black_box(1000);
            (0..n).fold(0.0f32, |a: f32, b: f32| a + b)
        })
    }

    #[bench]
    fn bench_add_float_builtin(b: &mut Bencher) {
        b.iter(|| {
            let n = test::black_box(1000);
            (0..n).fold(0.0f32, |a: f32, b: f32| __addsf3(a, b))
        })
    }

    #[bench]
    fn bench_add_double_normal(b: &mut Bencher) {
        b.iter(|| {
            let n = test::black_box(1000);
            (0..n).fold(0.0f64, |a: f64, b: f64| a + b)
        })
    }

    #[bench]
    fn bench_add_double_builtin(b: &mut Bencher) {
        b.iter(|| {
            let n = test::black_box(1000);
            (0..n).fold(0.0f64, |a: f64, b: f64| __addsd3(a, b))
        })
    }
}
